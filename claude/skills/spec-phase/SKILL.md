...
## Rules (must hold even outside this skill)

- Starting a phase authorizes its commits; it does **not** authorize push or PR-creation —
  those need a separate explicit go-ahead every time (see Do not, below).
...
4. Report to the user: phase complete, N commits, gate results, and ask explicitly whether
   to push + open the PR (stacked: phase-1 → `main`, phase-N → phase-(N-1)'s branch).
