...
- Base: `main` for phase 1; the previous phase's branch for phase N (stacked).
...
- An **On completion** line: update checklist, update root `HANDOFF.md`, stop and ask before
  push/PR — this must be present verbatim on every phase, it's a hard rule, not optional
  boilerplate to skip for brevity.

## Step 4 — Assemble EXECUTION.md

Header structure to match (see any existing `specs/*/EXECUTION.md` for the exact shape):
- Title, link to `PLAN.md`.
