<!--
  TEMPLATE — the ONLY domain-model content that belongs in a project's CLAUDE.md. The full
  rulebook lives at docs/DOMAIN-RULEBOOK.md (see `rulebook.md` in this directory) and is read
  on demand by the domain-modeling and grill-with-docs skills; CLAUDE.md is loaded every
  session, so it carries only what binds an agent that never triggers one of those skills.

  DELETE this comment block before appending the section to CLAUDE.md.
  ...
  Adding rules here is how CLAUDE.md bloats again. New rules go in DOMAIN-RULEBOOK.md unless
  an agent writing ordinary code or copy needs them.
-->
